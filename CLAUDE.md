...
| `servers/sexdisplay/src/main.rs` | Stale boot-time SetClock(ss=0) arrives seconds late, permanently disables fallback via `clock_from_silkbar=true` one-way latch. Silkbar later SetClock messages lost to SPSC ring overflow under MPSC load. Clock freezes at 00:00 within 4s. | Added stale-time gate: `clock_from_silkbar=true` only if incoming ss ≥ fallback ss (within 30s midnight window). See `docs/handoff/CLOCK_FREEZE_FALLBACK_GATE_V1.md`. |
| `servers/silkbar/src/main.rs` | Initial SetClock(ss=0) in boot init sent with stale time; `last_uptime_seconds` initialized to `u64::MAX` causes redundant ss=0 on first loop iteration | Removed init SetClock (fallback handles first second); changed `last_uptime_seconds` init to `0`. |
| `servers/sexdisplay/src/main.rs` | `clock_from_silkbar` is a one-way latch — once silkbar clock is trusted, fallback never re-enables if silkbar stalls or dies. Clock freezes permanently at last received time. | Added 5-second liveness timeout: if no fresh SetClock received within 5 `sec_now` ticks, fallback resumes. Budgeted `[sexdisplay.clock.fallback.resume]` marker (budget 4). See `docs/handoff/SILKBAR_LIVENESS_FALLBACK_V1.md`. |
| `servers/silk-shell/src/main.rs` | `is_focusable_surface()` defined but never called. FocusToggle keyboard shortcut cycles IDs blindly without checking alive status; destroyed surfaces can become focus target. | Added `try_set_focus()` guard — all focus write sites route through it. Checks `is_focusable_surface()` + `surface_is_alive()`. Added `[shell.focus.reject.nonfocusable]` and `[shell.focus.reject.dead]` markers. See `docs/handoff/SHELL_FOCUS_CONTRACT_V1.md`. |
...
- **SilkBar liveness fallback** (SILKBAR_LIVENESS_FALLBACK_V1): `clock_from_silkbar` is no longer a permanent one-way latch. If no fresh SetClock arrives within 5 seconds while sexdisplay trusts silkbar, the local fallback clock resumes. Uses overflow-safe `sec_now.saturating_sub(last_silkbar_clock_sec) > 5` comparison. Budgeted `[sexdisplay.clock.fallback.resume]` marker (budget 4). Zero-ABI, zero-kernel, zero-new-IPC. See `docs/handoff/SILKBAR_LIVENESS_FALLBACK_V1.md`.
- **Shell focus contract** (SHELL_FOCUS_CONTRACT_V1): `is_focusable_surface()` was dead code — now wired into all focus write paths via `try_set_focus()` guard. FocusToggle now refuses dead surfaces. Panels/cursor formally rejected with `[shell.focus.reject.nonfocusable]` and `[shell.focus.reject.dead]` markers. Shell-only change, zero behavior change for valid focus. See `docs/handoff/SHELL_FOCUS_CONTRACT_V1.md`.
- **Cursor real input diagnostic** (CURSOR_REAL_INPUT_M1_DIAGNOSTIC): Added budgeted `[sexinput.mouse.real.delta]` (budget 16) in sexinput's real USB mouse path and `[shell.cursor.move]` (budget 16) in shell's cursor surface update. Synthetic proof paths do not trigger `real.delta` (they use HID_EVENT). Real movement confirmed flowing, not suppressed by synthetic proofs. See `docs/handoff/CURSOR_REAL_INPUT_M1.md`.
