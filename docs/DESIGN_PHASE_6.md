...
## 🎯 Objective
Create a convenient, source-based distribution system called **SexSD**. Inspired by `xbps-src` (Void Linux) and BSD Ports, this system will automate the process of fetching, patching, and "lifting" drivers from existing Linux/BSD repositories into Sex Microkernel Protection Domains (PDs).
...
1.  **The Template:** A simple script (like `xbps-src` templates) that defines:
    *   `source`: URL to the Linux/BSD driver source.
    *   `patches`: Sex-specific patches to wire the driver to `DDE-Sex`.
    *   `pd_id`: The Protection Domain ID and PKU key assigned to the driver.
...
### 2. The Driver Registry (`drivers.json`)
- [ ] Build a database mapping hardware IDs to templates:
  - `0x10DE:0x2484` $\rightarrow$ `drivers/gpu/nvidia-3070`
  - `brcm,bcm2712-gpio` $\rightarrow$ `drivers/gpio/rpi5-gpio`
- [ ] Implement a `sex-probe` tool that identifies local hardware and recommends builds.

### 3. Initial "Lifting" Templates
- [ ] **`drivers/net/netbsd-stack`**: Lifts the NetBSD TCP/IP stack via Rump.
* [ ] **`drivers/gpu/nouveau-rtx`**: Lifts the Linux Nouveau driver for the 3070.
* [ ] **`drivers/storage/nvme-generic`**: Lifts the standard Linux NVMe driver.
* [ ] **`drivers/rpi5/vc7-gpu`**: Lifts the Raspberry Pi VideoCore VII driver.

### 4. The Sex Installer (`sex-provision`)
- [ ] A tool to generate a bootable UEFI/Pi image containing the kernel + the selected set of `.spd` driver images.
...
- **End-to-End Boot:** A "provisioned" image successfully boots on physical hardware and loads the required lifted drivers.
