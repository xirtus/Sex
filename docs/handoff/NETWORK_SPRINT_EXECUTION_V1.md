...
### Update 2026-05-19: e1000e CTRL.RST fix (SEXNET_E1000E_NIC_RESET_FOR_RX_V1)
- Root cause confirmed: HAL (source=2) fully enables e1000e RX/TX (RCTL.EN=1, TCTL.EN=1,
  IMS, RXDCTL, SRRCTL) before sexnet (source=3) takes ownership.
- Sexnet's ring address swap without CTRL.RST leaves e1000e internal state machine
  (descriptor fetch engine, FIFO, DMA engine) in inconsistent state.
- Fix: Added CTRL.RST (bit 26) device reset, IMC interrupt mask, RXDCTL/TXDCTL queue
  enable, bounded link poll before sexnet permanent ring programming.
- See: `SEXNET_E1000E_NIC_RESET_FOR_RX_V1.md`
...
- 2026-05-19: Phase G TCP handshake implementation completed. Added STOP review (`SEXNET_TCP_STATE_MACHINE_STOP_REVIEW_V1.md` — PASS REVIEW). Added TCP SYN build (src_port=7777, dst_port=80, seq=42, flags=SYN, data_offset=5, window=65535), TCP checksum (pseudo-header + TCP header), SYN TX via e1000e descriptor 5 (TDT=6), bounded DD poll (50M iterations). Added TCP SYN-ACK/RST handler in IPv4 RX path (proto=6): parses TCP header, validates checksum over pseudo-header, checks ports match, verifies ACK==local_seq+1 for SYN-ACK, transitions to ESTABLISHED on valid SYN-ACK or FAILED_RST on RST. Final ACK TX via descriptor 6 (TDT=7) with correct seq/ack. Proven markers: sexnet.tcp.syn.build.proof.done, sexnet.tcp.syn.tx.proof.done, sexnet.tcp.synack.rx.proof.done, sexnet.tcp.ack.tx.proof.done. Gate sexnet_tcp_handshake added to daily driver script. Minimal TCP state machine: CLOSED→SYN_SENT→ESTABLISHED (or FAILED_RST). One connection only. No TCP payload, no HTTP, no browser networking, no multi-connection table. Source ownership: sexnet source=3, coexisting with HAL diagnostic source=2 markers (preserved as-is). Phase G rollup updated in NETWORK_STACK_STATUS_ROLLUP_V1.md. Next: Phase H SEXNET_TCP_PAYLOAD_TX_STOP_REVIEW_V1.
