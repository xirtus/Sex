# Phase 5 Design: Hardware Enablement & sexdrive

## 🎯 Objective
Port the Sex Microkernel to physical hardware: **Raspberry Pi 5 (ARM64)** and **Intel x86_64 (X17R1)**. The goal is to move from QEMU to raw silicon while maintaining high-performance SASOS primitives and enabling complex sexdrive (NVIDIA GPU, Pi 5 Peripherals).

## 🏛 Architectural Vision: "DDE-Sex" (Device sexdrive Environment)

To avoid writing sexdrive from scratch, we will implement **DDE-Sex**, a compatibility layer that allows us to "lift" existing sexdrive from Linux and BSD into isolated Protection Domains (PDs).
...
3.  **Zero-Copy Hardware Paths:** sexdrive use "Memory Lending" to grant the hardware (DMA) direct access to application buffers, bypassing kernel mediation.
