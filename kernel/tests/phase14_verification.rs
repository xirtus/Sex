...
fn test_phase14_hardened_invariants() {
    ...
    assert!(
        sex_kernel::memory::allocator::GLOBAL_ALLOCATOR.verify_invariants(),
    ...
    assert!(
        sex_kernel::memory::pku::verify_isolation_invariant(pkru, 0),
        "Kernel isolation invariant violated"
    );
    ...
    assert!(
        current_pd.verify_ownership(1),
        "Capability ownership tracking lost"
    );
