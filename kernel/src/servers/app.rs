...
/// This uses the SexLibc emulation layer to interact with the OS.

pub fn posix_app_main(pd_id: u32) {
    serial_println!("APP: [PD {}] Starting POSIX application...", pd_id);

    // 1. Initialize Libc for the current PD
    let libc = SexLibc::new(pd_id);

    // 2. Demonstrate POSIX write() to stdout (FD 1)
    let msg = "Hello from Sex-Libc! This is a POSIX-like environment.\n";
    libc.write(1, msg.as_ptr(), msg.len()).expect("APP: Write failed");

    // 3. Demonstrate POSIX open() and read() from the VFS
    ...
    match libc.open("/disk0/config.json", 0) {
        Ok(fd) => {
            ...
            match libc.read(fd, buf.as_mut_ptr(), buf.len()) {
                Ok(_) => {
                    serial_println!("APP: Successfully read from VFS via Libc.");
                },
