...
fn proof_oversized_name() {
    ...
    let result = vfs::RAMFS.open(long, messages::RAMFS_O_CREATE, 0);
...
fn proof_oob_write() {
    ...
    let result = vfs::RAMFS.write(handle, 0, &big);
    ...
    let written = vfs::RAMFS.write(handle, 0, &ok_data)
    ...
    let result = vfs::RAMFS.write(handle, 1, &ok_data);
    assert!(result.is_err(), "[proof.4] post-boundary write must err");

    vfs::RAMFS.close(handle).expect("[proof.4] close failed");
...
fn proof_max_files() {
    ...
    for i in 0..messages::RAMFS_MAX_FILES {
        ...
        let h = vfs::RAMFS.open(name.as_bytes(), messages::RAMFS_O_CREATE, 0)
            .expect("[proof.6] max file create");
        handles.push(h);
    }

    // Next create should fail with ERR_FULL
    let result = vfs::RAMFS.open(b"file_overflow", messages::RAMFS_O_CREATE, 0);
    assert!(result.is_err(), "[proof.6] over max must err");
    assert_eq!(result.unwrap_err(), messages::ERR_FULL, "[proof.6] err code");

    // Cleanup
    for h in handles {
        vfs::RAMFS.close(h).expect("[proof.6] close failed");
    }
    assert_eq!(vfs::RAMFS.len(), 0, "[proof.6] all closed");
...
fn proof_close_reopen_persist() {
    ...
    let data = b"Persist!";
    vfs::RAMFS.write(handle, 0, data).expect("[proof.7] write failed");

    // Close releases handle but data persists
    vfs::RAMFS.close(handle).expect("[proof.7] close failed");

    // Reopen by name — should find same data
    let reopened = vfs::RAMFS.open(name, 0, 0).expect("[proof.7] reopen failed");
    let mut buf = [0u8; 64];
    let n = vfs::RAMFS.read(reopened, 0, &mut buf).expect("[proof.7] read failed");
    assert_eq!(n as usize, data.len(), "[proof.7] reopen read len");
    assert_eq!(&buf[..n as usize], data, "[proof.7] reopen data intact");

    vfs::RAMFS.close(reopened).expect("[proof.7] close reopened failed");
