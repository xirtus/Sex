#!/bin/bash
# =================================================================
# SexOS Phase 19: Substrate Hardening (Direct Path Force)
# =================================================================
set -euo pipefail

echo "─── Step 0: Identifying Nightly Path ───"
# Find the actual path to the nightly cargo binary
NIGHTLY_CARGO=$(rustup which cargo --toolchain nightly)
echo "Using Nightly Cargo at: $NIGHTLY_CARGO"

# Ensure core source is present
...
echo "─── Step 4: Atomic Synthesis (Absolute Path) ───"
# Run with the absolute path to the nightly cargo
$NIGHTLY_CARGO build \
