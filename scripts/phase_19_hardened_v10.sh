#!/bin/bash
# =================================================================
# SexOS Phase 19: Substrate Hardening (Limine 0.6.x API + Crate Fix)
# =================================================================
set -euo pipefail

echo "─── Step 1: Updating Dependencies (x86 crate) ───"
# We need to ensure the 'x86' crate is available for outb/inb
cd kernel
cargo add x86
cd ..

echo "─── Step 2: Restoring arch/mod.rs ───"
...
echo "─── Step 3: Implementing Serial Driver (UART 16550) ───"
...
echo "─── Step 4: Reconstructing Unitary Root (lib.rs) ───"
...
pub fn kernel_init() {
    ...
    serial_println!("[SexOS] Phase 19 Substrate Hardened (Limine 0.6.x API).");

    // Claim Framebuffer
    if let Some(fb_res) = FB_REQUEST.response().get() {
        if let Some(fb) = fb_res.framebuffers().first() {
            let ptr = fb.address.as_ptr().unwrap() as *mut u32;
            serial_println!("[SexOS] Framebuffer claimed: {}x{}", fb.width, fb.height);
...
echo "─── Step 5: Atomic Synthesis (RUSTC_BOOTSTRAP Force) ───"
